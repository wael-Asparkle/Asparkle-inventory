...
const CHANNEL_META = {
  'سعيدينيو': { bg: 'bg-indigo-50', text: 'text-indigo-700' },
  'دستور': { bg: 'bg-emerald-50', text: 'text-emerald-700' },
  'TikTok/Snapchat': { bg: 'bg-amber-50', text: 'text-amber-700' },
  'واتساب': { bg: 'bg-blue-50', text: 'text-blue-700' },
  'مباشر': { bg: 'bg-slate-50', text: 'text-slate-600' },
  'غير محدد': { bg: 'bg-slate-50', text: 'text-slate-600' },
};

function pct(a, b) {
  return b ? ((a / b) * 100).toFixed(0) : '0';
}
...
function customerKey(order) {
  return (order.customer?.phone || '').replace(/\s/g, '') || order.customer?.name || 'unknown';
}
...
function buildCustomers(orders) {
  ...
  orders.forEach((order) => {
    ...
    const skuBreakdown = order.skuBreakdown || {};
    ...
    customer.channels.add(order.channel || 'غير محدد');

    Object.entries(skuBreakdown).forEach(([sku, qty]) => {
      customer.skuCount[sku] = (customer.skuCount[sku] || 0) + Number(qty || 0);
    });
  });
  ...
  return Object.values(map).map((customer) => {
    ...
    const topSku = Object.entries(customer.skuCount).sort((a, b) => b[1] - a[1])[0]?.[0] || null;
    ...
    <div className={`border rounded-2xl overflow-hidden transition-all ${expanded ? 'border-indigo-200' : 'border-slate-100'}`}>
      ...
      >
        ...
        <div className="hidden md:flex items-center gap-6 flex-shrink-0">
          <div className="text-center">
            <p className="text-xs text-slate-400 font-bold">الطلبات</p>
            <p className={`text-lg font-black ${customer.ordersCount >= 2 ? 'text-indigo-600' : 'text-slate-700'}`}>{customer.ordersCount}</p>
          </div>
          <div className="text-center">
      ...
      {expanded && (
        <div className="bg-slate-50 border-t border-slate-100 px-5 py-4 space-y-4">
                ...
                {Object.entries(customer.skuCount)
                  ...
                  .map(([sku, qty]) => (
                    <div key={sku} className="bg-white border border-slate-200 rounded-xl px-3 py-2 flex items-center gap-2">
                      <span className="text-xs font-black text-slate-700">{PRODUCT_CATALOG[sku]?.name || sku}</span>
                      <span className="text-xs font-black bg-indigo-100 text-indigo-700 px-1.5 py-0.5 rounded-lg">×{qty}</span>
                    </div>
                  ))}
              </div>
            </div>
          ...
          <div>
            <p className="text-xs font-black text-slate-500 mb-2 flex items-center gap-1">
              <Calendar size={12} /> سجل الطلبات ({customer.ordersCount})
            </p>
            <div className="space-y-2">
              {customer.orders.map((order, index) => {
                const chMeta = CHANNEL_META[order.channel] || CHANNEL_META['غير محدد'];
                return (
                  <div key={order.reference || `${order.date}-${index}`} className="bg-white border border-slate-100 rounded-xl px-4 py-3 flex items-center gap-4 flex-wrap">
                    <div className="flex items-center gap-2">
                      {index === 0 && <span className="text-xs bg-emerald-100 text-emerald-700 font-black px-1.5 py-0.5 rounded-md">آخر طلب</span>}
                    ...
                    <span className="flex items-center gap-1 text-xs text-slate-400 font-bold">
                      <Calendar size={11} /> {order.date?.slice(0, 10) || '—'}
                    </span>
                    <span className={`text-xs font-black px-2 py-0.5 rounded-lg ${chMeta.bg} ${chMeta.text}`}>{order.channel}</span>
                    <div className="flex flex-wrap gap-1">
                      {Object.entries(order.skuBreakdown).map(([sku, qty]) => (
                    ...
                    <span className="text-sm font-black text-emerald-600 mr-auto">{fmtR(order.total)}</span>
                  </div>
                );
              })}
            </div>
          ...
          {customer.ordersCount >= 2 && (
            <div className="bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-3 flex items-center gap-3">
              ...
              <p className="text-xs font-bold text-indigo-700">
                ...
                {' '}— متوسط قيمة الطلب: <span className="font-black text-indigo-800">{fmtR(Math.round(customer.totalSpent / customer.ordersCount))}</span>
              </p>
            </div>
          )}
        </div>
...
export default function CRMTab() {
  ...
  const stats = useMemo(() => ({
    ...
    totalRev: customers.reduce((sum, c) => sum + c.totalSpent, 0),
    avgLTV: customers.length ? Math.round(customers.reduce((sum, c) => sum + c.totalSpent, 0) / customers.length) : 0,
    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={item.label} className={`${item.bg} border ${item.border} rounded-2xl p-5`}>
            <div className="w-9 h-9 bg-white rounded-xl flex items-center justify-center shadow-sm mb-3">{item.icon}</div>
            <p className="text-xs font-black text-slate-400 mb-1">{item.label}</p>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {quickSegments.map((item) => (
            <button
              ...
              className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${segment === item.id ? `${item.bg} ${item.border}` : 'border-slate-100 hover:bg-slate-50'}`}
            >
              <div className="flex items-center gap-2">
                <span className={item.color}>{item.icon}</span>
                <span className={`text-sm font-black ${segment === item.id ? item.color : 'text-slate-600'}`}>{item.label}</span>
              </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm px-5 py-4 flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-48">
          ...
          />
          {search && (
            <button type="button" onClick={() => setSearch('')} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600">
              <X size={13} />
            </button>
          )}
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {[
            ...
            <button
              ...
              className={`text-xs font-black px-3 py-1.5 rounded-lg transition-all ${sortBy === id ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
            >
              {label}
            </button>
          ))}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map((customer) => <CustomerRow key={customer.key} customer={customer} />)}
        </div>
      )}
    </div>
  );
