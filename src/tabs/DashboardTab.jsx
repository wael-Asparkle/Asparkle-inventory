...
function SaudiRiyalIcon({ size = 18, className = '' }) {
  return (
...
function Money({ value, className = '', iconSize = 16 }) {
  return (
      ...
      {showForm && (
        <div className="bg-indigo-50 border border-indigo-200 rounded-2xl p-4 mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2 mb-3">
            ...
            <div>
              <label className="text-xs font-black text-slate-500 mb-1 block">المبلغ</label>
                  ...
                  {[...entries].sort((a, b) => String(b.month || '').localeCompare(String(a.month || ''))).map((e) => {
                    ...
                    return (
                      <div key={e.id} className="flex items-center justify-between bg-white rounded-lg px-3 py-2 border border-slate-100">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-black text-slate-700"><Money value={e.amount} /></span>
...
export default function DashboardTab() {
  ...
  useEffect(() => {
    ...
    const unsubCosts = onSnapshot(adCol(), (snap) => setAdCosts(snap.docs.map((d) => ({ id: d.id, ...d.data() }))));
    return () => {
      unsubOrders();
      unsubCosts();
    };
  ...
  const availableChannels = useMemo(() => {
    ...
    adCosts.forEach((c) => c.channel && channels.add(c.channel));
  ...
  const filteredAdCosts = useMemo(() => adCosts.filter((c) => {
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm px-8 py-5 flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-black text-slate-800 flex items-center gap-2"><Zap size={22} className="text-indigo-500" /> لوحة التحكم</h2>
          <p className="text-slate-400 text-xs mt-1">بيانات من Firestore · <span className="font-black text-indigo-500">{fmt(totalOrders)}</span> طلب ظاهر من أصل {fmt(loadedOrdersCount)}</p>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={() => exportCSV({ channelStats, skuStats, adCosts: filteredAdCosts })} className="flex items-center gap-2 text-xs font-black bg-slate-800 hover:bg-slate-900 text-white px-4 py-2 rounded-xl transition-colors"><Download size={14} /> تصدير CSV</button>
          <div className="flex items-center gap-2 text-xs text-emerald-600 font-black bg-emerald-50 px-3 py-1.5 rounded-xl border border-emerald-100"><div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />متصل بـ Firebase</div>
        </div>
      </div>

      <div className="bg-white border border-slate-200 rounded-3xl p-4 shadow-sm">
        <div className="flex flex-col lg:flex-row lg:items-end gap-3">
          <div className="flex-1"><label className="block text-xs font-black text-slate-500 mb-1">من تاريخ</label><input type="date" value={filters.from} onChange={(e) => setFilters((p) => ({ ...p, from: e.target.value, month: '' }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-300" /></div>
          <div className="flex-1"><label className="block text-xs font-black text-slate-500 mb-1">إلى تاريخ</label><input type="date" value={filters.to} onChange={(e) => setFilters((p) => ({ ...p, to: e.target.value, month: '' }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-300" /></div>
          <div className="flex-1">
            ...
            <select value={filters.month} onChange={(e) => setFilters((p) => ({ ...p, month: e.target.value, from: '', to: '' }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-300">
              ...
              {availableMonths.map((m) => { const [yy, mm] = m.split('-'); return <option key={m} value={m}>{MONTHS_AR[mm]} {yy}</option>; })}
            </select>
          </div>
          <div className="flex-1"><label className="block text-xs font-black text-slate-500 mb-1">القناة</label><select value={filters.channel} onChange={(e) => setFilters((p) => ({ ...p, channel: e.target.value }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-300"><option value="all">كل القنوات</option>{availableChannels.map((ch) => <option key={ch} value={ch}>{ch}</option>)}</select></div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard label="إجمالي الإيرادات" value={<Money value={totalRevenue} iconSize={22} />} sub={`${fmt(totalOrders)} طلب`} icon={<SaudiRiyalIcon size={20} className="text-emerald-600" />} bg="bg-emerald-50" border="border-emerald-100" trend={monthlyTrend} trendColor="#10b981" />
        ...
        <KpiCard label="ROAS الإجمالي" value={totalRoas ? `${totalRoas}x` : '—'} sub={totalCost > 0 ? <>إنفاق <Money value={totalCost} /></> : 'أضف تكاليف الإعلانات'} icon={<Target size={20} className="text-amber-600" />} bg="bg-amber-50" border="border-amber-100" />
        <KpiCard label="متوسط قيمة الطلب" value={totalOrders ? <Money value={Math.round(totalRevenue / totalOrders)} iconSize={22} /> : '—'} sub="لكل طلب" icon={<BarChart2 size={20} className="text-blue-600" />} bg="bg-blue-50" border="border-blue-100" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
          <div className="flex items-center gap-2 mb-5"><TrendingUp size={18} className="text-indigo-500" /><h3 className="font-black text-slate-800">الإيرادات per قناة + ROAS</h3></div>
          <div className="space-y-3">
              ...
              return (
                <div key={ch} className={`p-3 rounded-xl border ${meta.border} ${meta.bg}`}>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0"><p className={`text-xs font-black ${meta.text}`}>{ch}</p><p className="text-xs text-slate-400">{fmt(st.orders)} طلب · <Money value={st.revenue} /></p></div>
                    <div className="flex items-center gap-2">{cost > 0 && <span className="text-xs text-slate-400">إنفاق <Money value={cost} /></span>}{roas && <span className={`text-xs font-black px-2 py-1 rounded-lg whitespace-nowrap ${good ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-600'}`}>ROAS {roas}x</span>}</div>
                  </div>
          ...
          {totalCost > 0 && <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between"><span className="text-xs font-black text-slate-500">الإجمالي</span><div className="flex items-center gap-3"><span className="text-xs text-slate-400">إنفاق <Money value={totalCost} /></span><span className={`font-black text-sm px-3 py-1 rounded-xl ${Number(totalRoas) >= 2 ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-600'}`}>ROAS {totalRoas}x</span></div></div>}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
          <div className="flex items-center gap-2 mb-5"><PackageOpen size={18} className="text-indigo-500" /><h3 className="font-black text-slate-800">المبيعات per SKU</h3></div>
          {topSkus.length === 0 ? <div className="text-center py-10"><PackageOpen size={32} className="text-slate-200 mx-auto mb-2" /><p className="text-slate-400 text-sm">لا يوجد بيانات مطابقة للفلاتر الحالية</p></div> : (
            <div className="space-y-4">
              {topSkus.map(([sku, st], i) => {
                ...
                return (
                  <div key={sku}>
                    <div className="flex items-center justify-between mb-1.5"><div className="flex items-center gap-2">{i === 0 && <Award size={13} className="text-amber-400" />}<span className="text-sm font-black text-slate-700">{name}</span><span className="text-xs text-slate-300 font-mono">{sku}</span></div><div className="flex items-center gap-2"><span className="text-xs text-slate-400">{fmt(st.orders)} طلب</span><span className="text-xs font-black bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-lg">{fmt(st.units)} وحدة</span></div></div>
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
          <div className="flex items-center gap-2 mb-5"><CheckCircle size={18} className="text-indigo-500" /><h3 className="font-black text-slate-800">التحقق من الطلب</h3></div>
          <div className="flex gap-2 mb-4"><input value={awbInput} onChange={(e) => setAwbInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && verifyAWB()} placeholder="رقم الطلب..." className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-300" /><button onClick={verifyAWB} className="bg-indigo-600 hover:bg-indigo-700 text-white font-black text-sm px-5 py-2.5 rounded-xl transition-colors">بحث</button></div>
          {awbResult && <div className={`rounded-2xl p-4 border ${awbResult.found ? 'bg-emerald-50 border-emerald-200' : 'bg-slate-50 border-slate-200'}`}>{!awbResult.found ? <p className="text-sm font-bold text-slate-500 flex items-center gap-2"><AlertTriangle size={16} className="text-amber-400" /> الطلب غير موجود</p> : (() => {
            const o = awbResult.order;
            const meta = CHANNEL_META[o.channel] || CHANNEL_META['غير محدد'];
            return (
              <div className="space-y-3">
                <div className="flex items-center justify-between"><span className="text-sm font-black text-emerald-700">✅ {o.reference}</span><span className={`text-xs font-black px-2 py-0.5 rounded-lg ${meta.bg} ${meta.text}`}>{o.channel}</span></div>
                <div className="grid grid-cols-2 gap-2 text-xs">{[['العميل', o.customer?.name], ['المدينة', o.customer?.city], ['الإجمالي', <Money value={o.total} />], ['التاريخ', getOrderDate(o)], ['الدفع', o.paymentMethod]].map(([k, v]) => <div key={k} className="bg-white rounded-lg px-3 py-2 border border-slate-100"><p className="text-slate-400 font-bold">{k}</p><p className="text-slate-700 font-black mt-0.5">{v || '—'}</p></div>)}</div>
                {o.skuBreakdown && <div className="flex flex-wrap gap-1 mt-1">{Object.entries(o.skuBreakdown).map(([sku, qty]) => <span key={sku} className="bg-indigo-50 text-indigo-700 text-xs font-black px-2 py-0.5 rounded-lg">{PRODUCT_CATALOG[sku]?.name || sku} ×{qty}</span>)}</div>}
              </div>
            );
          })()}</div>}
        </div>
      ...
      <div className="bg-slate-50 rounded-2xl border border-slate-100 px-6 py-3 flex items-center gap-2"><Clock size={14} className="text-slate-400 flex-shrink-0" /><p className="text-xs text-slate-400 font-bold">البيانات من Firestore · الطلبات والتكاليف المعروضة تتغير حسب الفلاتر · ROAS = الإيراد ÷ مجموع تكاليف القناة</p></div>
    </div>
  );
}
