...
// ─── buildSnapshotAtDate: لا يشمل DAMAGE/MISSING (بتوين يخصمهم) ───
function buildSnapshotAtDate(movements, beforeDate) {
  ...
  sorted.forEach((m) => {
    ...
    switch (m.movementType) {
      ...
      // DAMAGE و MISSING لا يدخلان في حساب المخزون — بتوين يخصمهم
    }
  });
  return stock;
}
...
function exportCSV(rows, missingRecords) {
  const stockLines = [
    ['SKU', 'اسم المنتج', 'الافتتاحي', 'المخزون الحالي', 'المرتجعات', 'الصافي المتاح'],
    ...rows.map((r) => [r.sku, r.name, r.opening, r.current, r.returns, r.net]),
  ].map((r) => r.join(',')).join('\n');
  const missingLines = [
    '', 'سجل المفقودات',
...
export default function StockTab() {
  ...
  // snapshot: DAMAGE/MISSING لا يدخلان في الحساب
  ...
  // ✅ الصافي = المخزون الحالي من بتوين مباشرة (بتوين يخصم DAMAGE/MISSING من عنده)
  const rows = useMemo(() => Object.keys(PRODUCT_NAMES).map((sku) => ({
    ...
    net:     snapshot[sku]   ?? 0,
  })).sort((a, b) => a.name.localeCompare(b.name, 'ar')), [snapshot, openingMap, returnMap]);
  ...
  const stats = useMemo(() => ({
    ...
    damage:         Object.values(damageMap).reduce((a, b) => a + b, 0),
    missing:        Object.values(missingMap).reduce((a, b) => a + b, 0),
    pendingMissing: missingRecords.filter((m) => m.status !== 'compensated').length,
  }), [rows, damageMap, missingMap, missingRecords]);
    ...
    <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-8" dir="rtl">
      ...
      {loading ? (
        ...
        <>
          ...
          {/* جدول المخزون — بدون عمودي الدامج والمفقود */}
          <div className="bg-slate-50 rounded-2xl border border-slate-100 overflow-x-auto mb-4">
            <table className="w-full text-sm">
              <thead className="bg-slate-100 text-slate-600">
                <tr>
                  {['SKU', 'اسم المنتج', 'الافتتاحي', 'المخزون الحالي', 'المرتجعات', 'الصافي المتاح'].map((h) => (
          ...
          <p className="text-xs text-slate-400 font-bold mb-10 text-center">
            {selectedDate ? `📅 المخزون كما كان بتاريخ ${selectedDate}` : '✅ الصافي = المخزون الحالي من بتوين — DAMAGE/MISSING محسوبة من بتوين مباشرة'}
          </p>

          {/* ══════════════════════════════════════════
              القسم الثاني: الدامج والمفقود
          ══════════════════════════════════════════ */}
          <div className="border-t border-slate-200 pt-8">
            ...
            <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-black text-slate-800">سجل الدامج والمفقودات</h3>
                  <p className="text-slate-400 text-xs mt-0.5">للمتابعة والتوثيق — لا تؤثر على حساب المخزون</p>
                </div>
              </div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <button onClick={() => setShowLossSection(!showLossSection)}
                  className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold bg-slate-50 text-slate-600 hover:bg-slate-100 border border-slate-200 transition-all">
                  <BarChart2 size={15} /> عرض السجلات
                  {showLossSection ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                </button>
              </div>
            </div>

            {/* ملخص سريع للدامج والمفقودات */}
